  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === 'GET' && path === '/dashboard') {
        ...
        return htmlResponse(dashboardHTML(access.user), 200, headers);
      }
...
async function customerExport(request, env, headers, requestId) {
  ...
  const [tenantResult, voiceNotes, followUps, leads] = await env.MAQVORA_DB.batch([
    env.MAQVORA_DB.prepare('SELECT business_name FROM clients WHERE id = ?').bind(tenant.client_id),
    env.MAQVORA_DB.prepare(`
  ...
  const businessName = tenantResult.results?.[0]?.business_name || 'Maqtomate Customer';
...
async function ownerOverview(request, env, headers) {
  ...
  const [tenants, mrr, users, errors, voiceFleet] = await env.MAQVORA_DB.batch([
...
<main class="wrap"><div id="loading" class="notice">Loading your secure dashboard…</div>
<section id="owner-app" class="hidden"><nav class="tabs" aria-label="Owner dashboard sections"><button class="tab active" data-panel="overview" onclick="showPanel('overview')">Overview</button><button class="tab" data-panel="customers" onclick="showPanel('customers')">Customers</button><button class="tab" data-panel="payments" onclick="showPanel('payments')">Payment reviews</button><button class="tab" data-panel="security" onclick="showPanel('security')">Security</button></nav>
<section id="overview" class="panel active"><div class="intro"><h1>Owner control center</h1><p>Fleet-wide values are visible only to approved platform roles. Customer credentials are never displayed here.</p></div><div class="grid"><div class="card"><div class="label">Active customers</div><div class="value" id="tenants">0</div></div><div class="card"><div class="label">Platform & dashboard users</div><div class="value" id="users">0</div></div><div class="card"><div class="label">Monthly recurring revenue</div><div class="value" id="mrr">Rs 0</div></div><div class="card"><div class="label">Errors — 7 days</div><div class="value" id="ownererrors">0</div></div><div class="card"><div class="label">Voice notes — 7 days</div><div class="value" id="ownervoice">0</div></div><div class="card"><div class="label">Follow-ups — 7 days</div><div class="value" id="ownerfollowups">0</div></div><div class="card"><div class="label">Qualified leads — 7 days</div><div class="value" id="ownerleads">0</div></div></div></section>
